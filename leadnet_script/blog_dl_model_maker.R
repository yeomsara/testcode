...
query <- "select id, header, text from all_blog_raw where view > 0" ## 쿼리작성
query1 <- "select id from blog_like"## 쿼리작성
...
tr.word <- foreach(i=1:length(train[,1]), .combine=rbind, .packages=c("ff","RMeCab")) %do% {extract_nouns(i, train, 3)}      
te.word <- foreach(i=1:length(test[,1]), .combine=rbind, .packages=c("ff","RMeCab")) %do% {extract_nouns(i, test, 3)}      
