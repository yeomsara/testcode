...
#"https://cran.r-project.org/src/contrib/Archive/h2o/h2o_3.10.3.3.tar.gz"
...
if ((length(args) > 0) & (grepl("^[0-9]{4}-[0-9]{2}-[0-9]{2}$", args[1]))) {
  curdate <- args[1]
} else {
  curdate <- as.character(Sys.Date())
}

query <- paste("select id, text from news_raw where date = '",curdate,"' and complete=0", sep="") ## 쿼리작성
#query <- paste("select id, text from news_raw where  complete=0", sep="") ## 쿼리작성
...
if (length(data[,1]) > 0){
  ...
  load("/home/ztc-linux/R/model/dl2.RData")
  ...
  # }
  pred_result$yn[pred_result$percent1 > 90] <- 1
  ...
  #   id <- pred_result$id[i]
  # }
  dbGetQuery(con,paste0("update news_raw set complete=1 where id in(",id,")"))
  dbGetQuery(con,"set names utf8")
  dbWriteTable(con, "news_recom", pred_result, append=T, row.names=F)
  dbDisconnect(con)
}
