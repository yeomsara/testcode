...
# FROM all_blog_raw A                                                
# LEFT OUTER JOIN blog_term_freq B on A.id = B.id                    
# WHERE A.date between '",from,"' and '",to,"' and B.id is null") ## 쿼리작성
query <- paste("select id, header, text from all_blog_raw where regdate > '",from,"' and regdate < '",to,"'", sep="") ## 쿼리작성
...
if (length(data) != 0){ ## 데이터 존재 시
  ...
  for (i in 1:length(data[,1])){
    ...
    tryCatch({
      ...
      png(paste0("R/png/wordcloud_blog_",as.character(data$id[i]),".png"), width=500,height=420,bg="transparent")
      wordcloud(c$term, c$termFreq, min.freq=1, scale=c(9,.1), random.order=T, colors=pal, rot.per=.15, family="BM DoHyeon")
      dev.off()
      
      ftpUpload(what = paste0("R/png/wordcloud_blog_",as.character(data$id[i]),".png"),
                to = paste0("ftp://192.168.0.21//home/ftp/var/etc/wordcloud_blog_",as.character(data$id[i]),".png"),
                verbose = TRUE,
  ...
  dbWriteTable(con, "blog_term_freq",upload, append=T, row.names=F)
  dbDisconnect(con)
}
