...
if(length(args) > 0) {
  ...
  query <- paste("select * from blog_raw where date = '",args[1],"' and complete in (1,2)", sep="") ## 쿼리작성

} else {
  cat(sprintf("execute for date : %s\n", curdate))
  query <- paste("select * from blog_raw where date = '",curdate,"' and complete in (1,2)", sep="") ## 쿼리작성
}
cat(query)
#query <- paste("select * from blog_raw where date = '",curdate,"' and complete in (1,2)", sep="") ## 쿼리작성
...
require(RMySQL)
con <- dbConnect(MySQL(), user="root", password="20110104", dbname="leadnet", host="192.168.0.21")
data1 <- dbGetQuery(con,paste0("SELECT * FROM blog_raw WHERE id in(",temp1,")"))
dbDisconnect(con)
...
targ_tb <- c("blog_ext_info","blog_like","blog_recom",
             "blog_summary" ,"blog_term_freq","blog_raw","all_blog_raw")
...
dbSendQuery(con,paste0("update blog_raw set complete = 2 where id in (",id,")"))
dbDisconnect(con)
