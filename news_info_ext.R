...
#query <- paste("select id, text from news_raw")
require(RMySQL)
...
for (j in 1:length(text[,1])){
  tryCatch({
    ...
    ## 금액
    money1 <- na.omit(as.data.frame(str_extract(text2, "[0-9]+(만원|억원|천원|조)")))
    ...
    # 규모
    scale1 <- as.character(na.omit(str_extract(text1, "[0-9]+(톤|㎞|km|㎥|㎡|여평|평|명|여명)")))
    scale <- unique(scale1)
    ...
    temp <- as.data.frame(cbind(locale, money, scale, period,text$id[j]))
...
dbWriteTable(con, "news_ext_info", ext_info, append=T, row.names=F)
dbDisconnect(con)
